...
async def _get_assistants(chat_id: int) -> Dict[str, int]:
    _notes = assisdb.find_one({"chat_id": chat_id})
    if not _notes:
        return {}
    return _notes["notes"]
...
async def get_assistant(chat_id: int, name: str) -> Union[bool, dict]:
    ...
    _notes = await _get_assistants(chat_id)
    if name in _notes:
        return _notes[name]
    else:
        return False

async def getassistant(chat_id: int, name: str) -> Union[bool, dict]:
    name = name.lower().strip()
    _notes = await _get_assistants(chat_id)
    if name in _notes:
        return _notes[name]
