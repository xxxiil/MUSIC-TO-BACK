...
from ELLIOT.queues import QUEUE, add_to_queue, get_queue, clear_queue

from ELLIOT.main import call_py, bot as ELLIOT, Test, call_py2, call_py3, call_py4, call_py5
...
@ELLIOT.on_message(filters.user(SUDO_USERS) & filters.command(["vcraid"], prefixes=HNDLR))
async def vcraid(_, e: Message):
    ...
    if inp:
        ELLIOT = await e.reply_text("**بدا  الرديو**")
        link = f"https://ELLIOT-robot.github.io/{aud[1:]}"
        dl = aud
        songname = aud[18:]
        if chat_id in QUEUE:
...
@ELLIOT.on_message(filters.user(SUDO_USERS) & filters.command(["vraid"], prefixes=HNDLR))
async def vraid(_, e: Message):
    ...
    if inp:
        ELLIOT = await e.reply_text("**Starting Raid**")
        link = f"https://ELLIOT-robot.github.io/{aud[1:]}"
        songname = aud[18:]
        if chat_id in QUEUE:
            pos = add_to_queue(chat_id, songname, dl, link, "Audio", 0)
...
@ELLIOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidend"], prefixes=HNDLR))
...
@ELLIOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidpause"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
...
@ELLIOT.on_message(filters.user(SUDO_USERS) & filters.command(["raidresume"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
