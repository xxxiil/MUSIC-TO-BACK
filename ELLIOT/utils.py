import os
import asyncio
from ELLIOT.main import bot, call_py, call_py2, call_py3, call_py4, call_py5
from config import NEXT_IMG
from pytgcalls.types import Update
from pytgcalls.types.input_stream import AudioPiped, AudioVideoPiped
from ELLIOT.queues import QUEUE, clear_queue, get_queue, pop_an_item
...
async def stream_end_handler(_, u: Update):
    if isinstance(u, StreamAudioEnded):
        ...
        if op==1:
           await bot.send_message(chat_id, "✅ **انتهت الموسيقي.**")
        elif op==2:
           await bot.send_message(chat_id, "❌ **حدث خطأ اثناء**\n\n» **محو** __قوائم الانتظار__ **ومغادرة الدردشه.**")
        else:
         await bot.send_photo(chat_id, f"{NEXT_IMG}", caption=f"💡 **تخطي الي المسار التالي**\n\n🏷 **الاسم:** [{op[0]}]({op[1]}) | `{op[2]}`\n💭 **الدردشه:** `{chat_id}`", reply_markup=keyboard)
    else:
       pass
...
async def stream_end_handler(_, u: Update):
    if isinstance(u, StreamAudioEnded):
        ...
        op = await skip_current_song(chat_id)
        if op==1:
           await bot.send_message(chat_id, "✅ **تم قطع اتصال الحساب المساعدt.**")
        elif op==2:
           await bot.send_message(chat_id, "❌ **حدث خطأ اثناء**\n\n» **محو** __قوائم الانتظار__ **ومغادرة الدردشه.**")
        else:
         await bot.send_photo(chat_id, f"{NEXT_IMG}", caption=f"💡 **تخطي الي المسار التالي**\n\n🏷 **الاسم:** [{op[0]}]({op[1]}) | `{op[2]}`\n💭 **الدردشه:** `{chat_id}`", reply_markup=keyboard)
